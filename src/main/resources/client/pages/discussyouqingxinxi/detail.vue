...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussyouqingxinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussyouqingxinxiCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussyouqingxinxi', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussyouqingxinxi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussyouqingxinxi');
			...
			async init(){
				...
				let res = await this.$api.info('discussyouqingxinxi', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussyouqingxinxi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussyouqingxinxi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
