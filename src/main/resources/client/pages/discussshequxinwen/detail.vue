...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussshequxinwenCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussshequxinwenCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussshequxinwen', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussshequxinwen');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussshequxinwen');
			...
			async init(){
				...
				let res = await this.$api.info('discussshequxinwen', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussshequxinwen', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussshequxinwen/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
